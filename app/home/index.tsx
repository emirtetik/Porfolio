...
const Home = () => {
  ...
  return (
    ...
    <div className="md:w-2/5 p-6 flex flex-col space-y-6">
      <Chatbot />
      <div className="p-2">
        <h1 className="text-[var(--text-white)] text-3xl md:text-4xl font-bold">Emir Tetik</h1>
        <h3 className="text-[var(--text-white)] text-xl md:text-2xl">Frontend & React Native Developer</h3>
        <p className="text-[var(--text-white)] mt-4 text-sm md:text-base ">{t("Home text")}</p>
        <div className="w-40 h-[1px] bg-black my-3" />
        <div className="flex gap-6">
          <Link href="https://github.com/emirtetik" target="_blank">
            <FaGithub className="text-[var(--text-black)] text-2xl md:text-3xl hover:text-white transition-colors" />
          </Link>
          <Link href="https://stackoverflow.com/users/20142369/frontmir" target="_blank">
            <FaStackOverflow className="text-[var(--text-black)] text-2xl md:text-3xl hover:text-orange-600 transition-colors" />
          </Link>
          <Link href="https://www.linkedin.com/in/emir-tetik/" target="_blank">
            <FaLinkedin className="text-[var(--text-black)] text-2xl md:text-3xl hover:text-blue-700 transition-colors" />
          </Link>
        </div>
      </div>
    </div>
    <div className="md:w-3/5 flex flex-col justify-center space-y-4">
      {NavData.map((item) => (
        <div key={item.id} className="mb-2 md:mb-4">
          <Link href={item.url}>
            <div className="font-prata text-[var(--text-white)] text-center uppercase transition-all duration-500 ease-out transform hover:translate-x-5 hover:-skew-x-6 
